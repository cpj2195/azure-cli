...
class MigrationScenarioTest(ScenarioTest):
    ...
    def _test_server_migration(self, database_engine):
        ...
        properties_filepath = os.path.join(curr_dir, 'migrationVNet_PGupgrades.json').replace('\\', '\\\\')
         
        # test check migration name availability -success
        result = self.cmd('{} server migration check-name-availability --subscription {} --resource-group {} --name {} --migration-name {} '
        ...
        result = self.cmd('{} server migration create --subscription {} --resource-group {} --name {} --migration-name {} --properties {} '
        ...
        result = self.cmd('{} server migration list --subscription {} --resource-group {} --name {} --filter Active'
        ...
        result = self.cmd('{} server migration list --subscription {} --resource-group {} --name {}'
                          .format(database_engine, target_subscription_id, target_resource_group_name, target_server_name)).get_output_in_json()

        # test show migration - success
        result = self.cmd('{} server migration show --subscription {} --resource-group {} --name {} --migration-name {}'
                          .format(database_engine, target_subscription_id, target_resource_group_name, target_server_name, migration_name)).get_output_in_json()

        # test update migration - error - no param
        result = self.cmd('{} server migration update --subscription {} --resource-group {} --name {} --migration-name {}'
        ...
        result = self.cmd('{} server migration delete --subscription {} --resource-group {} --name {} --migration-name {} --yes'
                          .format(database_engine, target_subscription_id, target_resource_group_name, target_server_name, migration_name)).get_output_in_json()
